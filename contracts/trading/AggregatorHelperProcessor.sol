...
contract AggregatorHelperProcessor is ICalldataProcessor {
    ...
    function processCalldata(uint256 amount, bytes calldata data) external pure returns (bytes memory processedCalldata) {
        ...
        if (selector == IAggregatorHelper.swapExactOutput.selector) {
            ...
            ) = abi.decode(data[4:], (address, address, uint256, uint256, address, bytes, address, bytes, uint256));
            amountOut = amount;
            return abi.encodeCall(IAggregatorHelper.swapExactOutput, (src, dst, amountIn, amountOut, aggregator, aggregatorCalldata, scrapRouter, scrapCalldata, scrapAmountOffset));
        }
        else {
